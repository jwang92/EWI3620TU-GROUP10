...
public class LeftWing extends GameObject implements VisibleObject { 										
	private Enemy enemy;
	...
	public LeftWing(double x, double y, double z,boolean tex, String modelName, MainClass mclass){
		...
		try {
			if(texture){
				m = OBJLoader.loadTexturedModel((new File("3d_object/Bathos/LeftWing.obj")));
			}
			else{
				m = OBJLoader.loadModel((new File("3d_object/Bathos/LeftWing.obj")));
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
