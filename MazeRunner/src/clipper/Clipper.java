...
public class Clipper extends ClipperBase {
	...
	public void execute(ClipType clipType, List<PolygonClipper> solution, PolyFillType subjFillType, PolyFillType clipFillType) {
		if(m_ExecuteLocked)
	...
	public void execute(ClipType clipType, List<PolygonClipper> solution) {
	...
	public static void reversePolygons(List<PolygonClipper> polys) {
		for(PolygonClipper poly : polys)
			poly.reverse();
	}

	public static boolean orientation(PolygonClipper poly) {
		return area(poly) >= 0;
	}
	...
	private void BuildResult(List<PolygonClipper> polyg) {
		polyg.clear();
		for(int i = 0; i < m_PolyOuts.size(); i++) {
			OutRec outRec = m_PolyOuts.get(i);
			if(outRec.pts == null)
			...
			if(cnt < 3)
				continue;
			PolygonClipper pg = new PolygonClipper();
			for(int j = 0; j < cnt; j++) {
				pg.add(p.pt);
				p = p.prev;
			}
	...
	private static boolean FullRangeNeeded(PolygonClipper pts) {
	...
	public static double area(PolygonClipper poly) {
	...
	static PolygonClipper BuildArc(IntPoint pt, double a1, double a2, double r, double limit) {
		...
		PolygonClipper result = new PolygonClipper();
	...
	private static class PolyOffsetBuilder {
		private List<PolygonClipper> m_p;
		private PolygonClipper currentPoly;
		...
		public PolyOffsetBuilder(List<PolygonClipper> pts, List<PolygonClipper> solution, boolean isPolygon, double delta, JoinType jointype, EndType endtype) {
			this(pts, solution, isPolygon, delta, jointype, endtype, 0);
		}

		public PolyOffsetBuilder(List<PolygonClipper> pts, List<PolygonClipper> solution, boolean isPolygon, double delta, JoinType jointype, EndType endtype, double limit) {
			...
			for(m_i = 0; m_i < pts.size(); m_i++) {
				...
				currentPoly = new PolygonClipper();
				if(isPolygon || forceClose) {
					m_k = len - 1;
			...
			if(delta > 0) {
				...
				PolygonClipper outer = new PolygonClipper();
			...
			if((normals.get(m_k).X * normals.get(m_j).Y - normals.get(m_j).X * normals.get(m_k).Y) * m_delta >= 0) {
				if((normals.get(m_j).X * normals.get(m_k).X + normals.get(m_j).Y * normals.get(m_k).Y) < 0.985) {
					...
					if(m_delta > 0 && a2 < a1)
						...
						a2 -= Math.PI * 2;
					PolygonClipper arc = BuildArc(m_p.get(m_i).get(m_j), a1, a2, m_delta, Limit);
	...
	public static List<PolygonClipper> offsetPolygons(List<PolygonClipper> poly, double delta, JoinType joinType, double miterLimit, boolean autoFix) {
		List<PolygonClipper> result = new ArrayList<PolygonClipper>();
	...
	public static List<PolygonClipper> offsetPolygons(List<PolygonClipper> poly, double delta, JoinType jointype, double miterLimit) {
		return offsetPolygons(poly, delta, jointype, miterLimit, true);
	}

	public static List<PolygonClipper> offsetPolygons(List<PolygonClipper> poly, double delta, JoinType jointype) {
		return offsetPolygons(poly, delta, jointype, 0, true);
	}

	public static List<PolygonClipper> offsetPolygons(List<PolygonClipper> poly, double delta) {
		return offsetPolygons(poly, delta, JoinType.jtSquare, 0, true);
	}

	public static List<PolygonClipper> offsetPolyLines(List<PolygonClipper> lines, double delta, JoinType jointype, EndType endType, double limit) {
		List<PolygonClipper> result = new ArrayList<PolygonClipper>();
		...
		List<PolygonClipper> pts = new ArrayList<PolygonClipper>();
		for(int i = 0; i < pts.size(); ++i) {
		...
		if(endType == EndType.CLOSED) {
			int sz = pts.size();
			for(int i = 0; i < sz; ++i) {
				PolygonClipper line = new PolygonClipper(pts.get(i));
				line.reverse();
				pts.add(line);
			}
	...
	public static List<PolygonClipper> simplifyPolygon(PolygonClipper poly) {
		return simplifyPolygon(poly, PolyFillType.EVENODD);
	}

	public static List<PolygonClipper> simplifyPolygon(PolygonClipper poly, PolyFillType fillType) {
		List<PolygonClipper> result = new ArrayList<PolygonClipper>();
	...
	public static List<PolygonClipper> simplifyPolygons(List<PolygonClipper> polys) {
		return simplifyPolygons(polys, PolyFillType.EVENODD);
	}

	public static List<PolygonClipper> simplifyPolygons(List<PolygonClipper> polys, PolyFillType fillType) {
		List<PolygonClipper> result = new ArrayList<PolygonClipper>();
		Clipper c = new Clipper();
		c.setForceSimple(true);
	...
	public static PolygonClipper cleanPolygon(PolygonClipper poly) {
		return cleanPolygon(poly, 1.414);
	}

	public static PolygonClipper cleanPolygon(PolygonClipper poly, double distance) {
	...
	public static List<PolygonClipper> cleanPolygons(List<PolygonClipper> polys) {
		return cleanPolygons(polys, 1.415);
	}

	public static List<PolygonClipper> cleanPolygons(List<PolygonClipper> polys, double distance) {
		List<PolygonClipper> result = new ArrayList<PolygonClipper>();
	...
	public static void polyTreeToPolygons(PolyTree polytree, List<PolygonClipper> polygons) {
		polygons.clear();
		addPolyNodeToPolygons(polytree, polygons);
	}

	public static void addPolyNodeToPolygons(PolyNode polynode, List<PolygonClipper> polygons) {
